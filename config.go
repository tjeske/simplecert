...
func CheckConfig(c *Config) error {
	...
	if c.WillRenewCertificate == nil && (c.HTTPAddress != "" || c.TLSAddress != "") {
		log.Warn("no WillRenewCertificate handler specified, to handle graceful server shutdown!")
	}
	if c.DidRenewCertificate == nil && (c.HTTPAddress != "" || c.TLSAddress != "") {
		log.Warn("no DidRenewCertificate handler specified, to bring the service back up after renewing the certificate!")
	}
	if c.FailedToRenewCertificate == nil {
		log.Warn("no FailedToRenewCertificate handler specified! Simplecert will fatal on errors!")
	}
